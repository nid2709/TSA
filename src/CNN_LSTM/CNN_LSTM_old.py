...
def split_data_by_station(df):
    ...
    train_df = df[df[STATION_COLUMN] == train_station].sort_index()
    val_df = df[df[STATION_COLUMN] == val_station].sort_index()
    test_df = df[df[STATION_COLUMN] == test_station].sort_index()
...
def prepare_cnn_lstm_data(df, input_seq_length=24, output_seq_length=6, batch_size=32):
    ...
    train_parts, model_features = build_model_parts(train_parts, station_ids)
    val_parts, _ = build_model_parts(val_parts, station_ids)
    test_parts, _ = build_model_parts(test_parts, station_ids)

    train_parts, val_parts, test_parts = scale_data(train_parts, val_parts, test_parts)

    X_train, y_train = create_sequences(train_parts, model_features, input_seq_length, output_seq_length)
    X_val, y_val = create_sequences(val_parts, model_features, input_seq_length, output_seq_length)
    X_test, y_test = create_sequences(test_parts, model_features, input_seq_length, output_seq_length)
...
def plot_predictions(actuals, predictions, forecast_step=1, max_plot_points=1200):
    ...
    if len(x_values) > max_plot_points:
        plot_indices = np.linspace(0, len(x_values) - 1, max_plot_points, dtype=int)
        x_values = x_values[plot_indices]
        actual_values = actual_values[plot_indices]
        predicted_values = predicted_values[plot_indices]
...
def plot_forecast_comparison(actuals, predictions):
    ...
    if output_seq_length > 1:
        plot_predictions(actuals, predictions, forecast_step=output_seq_length)
...
def run_cnn_lstm_model(
    ...
    train_loader, val_loader, test_loader, input_size = prepare_cnn_lstm_data(
        df,
    ...
    if show_prediction_plot:
        plot_forecast_comparison(actuals, predictions)

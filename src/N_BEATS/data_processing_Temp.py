...
def resample_with_15_min(feature_df):
    print("\n========== Data Resampling with 15 Minutes ==========")
    resample_freq = "15min"
...
def add_15_min_ahead_column(feature_df):
    print("\n========== Add 15 Minutes Ahead Temperature Column ==========")
    print(feature_df.shape)

    feature_df = feature_df.sort_values(["station_id", "timestamp"]).reset_index(drop=True)

    target_df = feature_df[["station_id", "timestamp", "scd41_temperature"]].copy()

    target_df = target_df.rename(
        columns={
            "timestamp": "actual_target_timestamp",
            "scd41_temperature": "target_temperature_15min",
        }
    )
    ...
    feature_df = feature_df.dropna(subset=["target_temperature_15min"]).reset_index(drop=True)
...
def data_preprocessing_Temp():
    ...
    feature_df, numeric_cols = clipping_outliers(feature_df)

...
from src.LSTM.LSTM_config import get_lstm_results_dir
...
def plot_mc_dropout_uncertainty(
    ...
    ax.set_title(
        f"MC Dropout Uncertainty for LSTM "
...
def run_mc_dropout_uq(
    ...
    # Runs MC Dropout uncertainty analysis using saved LSTM model outputs.
    """
    Main function to run MC Dropout UQ using results from run_lstm_model().
    """

    model = lstm_results["model"]
    X_test = lstm_results["X_test"]
    ...
    deterministic_predictions = lstm_results.get("predictions")
